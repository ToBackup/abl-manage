...
namespace ED.BLL.CellPort
{
    class CellHelperI : ICellHelper
    {
        ...
        public void FindColumnIndex(IEnumerable<Cell> columns)
        {
            ...
            {
                Cell cell = FindCell(columns, ToObtainColumn[i]);
        ...
        public Cell FindCell(IEnumerable<Cell> cols,string key)
        {
            Cell cell = null;
            foreach (Cell item in cols)
            {
                if (item == key)
                    return item;
            }
